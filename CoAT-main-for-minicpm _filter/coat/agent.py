...
class ScreenAgent(BaseAgent):
    ...
    def predict(self, step_data, save_dir, max_trials=5): 
        ...
        if not os.path.exists(cur_save_path):
            cur_anno = {}  # 如果文件不存在，初始化一个空的字典
            with open(cur_save_path, 'w') as f:
                json.dump(cur_anno, f, indent=4)  # 创建一个新的空 JSON 文件
        else:
            cur_anno = json.load(open(cur_save_path, 'r'))  # 如果文件存在，加载内容
        if prev_step_id:
                ...
                if state == HTTPStatus.OK: 
                    try: 
                        ...
                        cur_anno['action_predict'][action_mode][self.screen_mode] = {
                            "ACTION": response["action"]['name'], "ARGS": response["action"]["args"]}
